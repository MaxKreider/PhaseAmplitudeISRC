...
%function
F_c = @(t,u) Tdawg/Tmax*[u(1)-u(1)^3-u(2)+delta*(u(3)-u(1)); 
              u(1);
              1/fac*(u(3)-u(3)^3-u(4)+eps)-delta*(u(3)-u(1));
              1/fac*(u(3)+eps)];

%solve for system
x02 = [0.842674 -1.03114 -1.0531 1.14562];
[Tc,Uc] = ode113(F_c,0:dt:Tmax*1,x02,opts);
...
% 
% %compute unperturbed period T_p
% [~,loc]=findpeaks(Uc(:,3),'MinPeakHeight',0);
% timerz = Tc(loc);
% Tp = mean(diff(timerz))
...
figure(2)
hold on
plot(Tc,U(:,3),'k*','LineWidth',4)
plot(Tc,U(:,4),'m*','LineWidth',4)
...
plot(T,Uc(:,4),'color',[255, 182, 193 256]/256,'LineWidth',4)
xlabel('time t')
ylabel('w(t), z(t)')
set(gca,'fontsize',15)
box on
...
integrand=zeros(1,length(t));
for i=1:length(t)
    integrand(i) = exp(-kappa_2*t(i))*dot(I0K_2(:,i),-G(:,i));
end
entry = cumtrapz(t,integrand);
entry = entry(end);
...
for j=1:length(t)
    if j>1
    integrand = exp(-kappa_2*t(1:j)).*dot(I0K_2(:,1:j),-G(:,1:j));
    integrand = cumtrapz(dt,integrand);
    integrand = integrand(end);
    else
        integrand=0;
...
plot(T,Uc(:,4)-U(:,4),'m*','LineWidth',12)

%plot it
figure(3)
plot(t(1:end-1),sol_1(1,1:end-1),'color',[.7 .7 .7],'marker','*','LineWidth',6)
plot(t(1:end-1),sol_1(2,1:end-1),'color',[255, 182, 193 256]/256,'marker','*','LineWidth',6)
xlim([0 Tmax])
box on
axis square
title('iSRC 1')
...
%% compute quantitative comparison (1st oscillator)
...
%compute E_delta
E_delta_osc_1 = 1/delta*numer/denomer


%% compute quantitative comparison (2nd oscillator)

%numerator
numer = cumtrapz(dt,abs(Uc(:,3) - U(:,3) - sol_2(1,:)').^2 + abs(Uc(:,4) - U(:,4) - sol_2(2,:)').^2);
numer = sqrt(numer(end));

%mean value
meaner1 = cumtrapz(dt,Uc(:,3))/T_p;
meaner1 = meaner1(end);
meaner2 = cumtrapz(dt,Uc(:,4))/T_p;
meaner2 = meaner2(end);

%denom
denomer = cumtrapz(dt,abs(Uc(:,3) - meaner1).^2 + abs(Uc(:,4) - meaner2).^2);
denomer = sqrt(denomer(end));

%compute E_delta
E_delta_osc_2 = 1/delta*numer/denomer
